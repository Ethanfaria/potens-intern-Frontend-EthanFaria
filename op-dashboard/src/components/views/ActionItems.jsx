...
export default function ActionItems() {
  ...
  const sorted = [
    ...
    ...actionItems.filter((i) => !!itemState[i.id]),
  ];
      ...
      <div className="mb-4 flex items-start sm:items-end justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Ranked by business impact · {done} of {total} resolved
          </p>
        </div>
      ...
      <div className="flex flex-col gap-3 sm:hidden">
        {sorted.map((item, index) => {
          ...
          return (
            ...
            >
              <div
                className={`flex items-center gap-3 px-4 py-2.5 border-b border-slate-200 ${isDone ? "bg-white border-l-[3px] border-l-slate-200" : cfg.row}`}
              >
              ...
              <div className="px-4 py-3 bg-white">
                <p
                  className={`text-sm font-semibold leading-snug ${isDone ? "text-slate-400 line-through" : "text-slate-900"}`}
                >
                  {item.title}
                </p>
                ...
                {!isDone && (
                  <div className="flex gap-2 mt-3">
                    <button
                      ...
                      className="flex-1 text-xs py-2 rounded-lg bg-teal-600 text-white font-bold
                        hover:bg-teal-700 active:scale-95 transition-all duration-150 shadow-sm"
                    >
                      Approve
                    </button>
                    <button
                      onClick={() => doAction(item.id, "held")}
                      className="flex-1 text-xs py-2 rounded-lg bg-white text-red-500 font-bold
                        border-2 border-red-400 hover:bg-red-500 hover:text-white
          ...
          {sorted.map((item, index) => {
            ...
            return (
              <div
                key={item.id}
                className={`
                  grid grid-cols-[60px_1fr_auto] gap-3 px-4 py-3.5
                  items-center border-b border-slate-200 last:border-0
                  transition-all duration-300
                  ${isDone ? "bg-white opacity-55 border-l-[3px] border-l-slate-200" : cfg.row}
                `}
              >
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  {isApproved && (
                    <span className="flex items-center gap-1 text-xs text-teal-600 font-bold whitespace-nowrap">
                      <MdCheckCircle size={14} /> Approved
                    </span>
                  )}
                  {isHeld && (
                    <span className="flex items-center gap-1 text-xs text-red-500 font-bold whitespace-nowrap">
                      <MdPause size={14} /> On Hold
                    </span>
                  )}
                  {!isDone && (
                    <>
                      <button
                        onClick={() => doAction(item.id, "approved")}
                        className="text-xs px-2.5 py-1.5 rounded-lg bg-teal-600 text-white font-bold
                          hover:bg-teal-700 active:scale-95 transition-all duration-150 shadow-sm"
                      >
                        Approve
                      </button>
                      <button
                        onClick={() => doAction(item.id, "held")}
                        className="text-xs px-2.5 py-1.5 rounded-lg bg-white text-red-500 font-bold
                          border-2 border-red-400 hover:bg-red-500 hover:text-white
                          active:scale-95 transition-all duration-150"
                      >
                        Hold
      ...
      <div className="hidden lg:block">
        <div className="rounded-xl border border-slate-200 overflow-hidden shadow-sm">
          <div className="grid grid-cols-[36px_88px_1fr_auto] gap-3 px-5 py-2.5 bg-slate-100 border-b border-slate-200">
            ...
            <span className="text-[10px] font-mono font-semibold text-slate-500 uppercase tracking-widest text-center">
              Priority
            </span>
            <span className="text-[10px] font-mono font-semibold text-slate-500 uppercase tracking-widest">
              Item
          ...
          {sorted.map((item, index) => {
            ...
            return (
              <div
                key={item.id}
                className={`
                  grid grid-cols-[36px_88px_1fr_auto] gap-3 px-5 py-3.5
                  items-center border-b border-slate-200 last:border-0
                  transition-all duration-300
                  ${isDone ? "bg-white opacity-55 border-l-[3px] border-l-slate-200" : cfg.row}
                `}
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  {!isDone && (
                    <>
                      ...
                      <button
                        onClick={() => doAction(item.id, "held")}
                        className="text-xs px-3 py-1.5 rounded-lg bg-white text-red-500 font-bold
                          border-2 border-red-400 hover:bg-red-500 hover:text-white
                          active:scale-95 transition-all duration-150"
                      >
                        Hold
