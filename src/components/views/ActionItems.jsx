...
const priorityConfig = {
  P1: {
    ...
    rowLowBW: "border-l-[3px] border-l-red-400", // flat variant — no bg tint
...
export default function ActionItems() {
  ...
  // Pending items always float to the top, sorted by priority
  const sorted = [
    ...translatedItems
  ...
  // j/k to navigate rows, a/h to approve or hold the focused item
    ...
    (e) => {
      ...
      if (e.key === "j" || e.key === "k") {
        ...
        setFocusedId((prev) => {
          ...
          if (e.key === "j") {
            return pendingItems[
              currentIndex <= 0 ? pendingItems.length - 1 : currentIndex - 1
            ].id;
          } else {
            return pendingItems[
              currentIndex === -1 || currentIndex >= pendingItems.length - 1
                ? 0
                : currentIndex + 1
            ].id;
          }
      ...
      if ((e.key === "a" || e.key === "h") && focusedId) {
        ...
        // After acting, auto-advance focus to the next pending item
        setFocusedId(() => {
  ...
  function rowBg(cfg, isDone) {
    if (isDone)
      return "bg-white dark:bg-slate-900/60 opacity-55 border-l-[6px] border-l-slate-200 dark:border-l-slate-600";
      ...
      {!lowBW && (
        <div className="hidden lg:flex items-center gap-3 mb-4 text-[11px] text-slate-400 dark:text-slate-500">
          <span className="flex items-center gap-1">
            <kbd className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-600 font-mono text-slate-500 dark:text-slate-400">
              j
            </kbd>
          </span>
          <span className="flex items-center gap-1">
          ...
          <span className="flex items-center gap-1">
            <kbd className="px-1.5 py-0.5 rounded bg-teal-50 dark:bg-teal-900/30 border border-teal-300 dark:border-teal-700 font-mono text-teal-600 dark:text-teal-400">
              a
            </kbd>
            approve
          </span>
          <span className="flex items-center gap-1">
            <kbd className="px-1.5 py-0.5 rounded bg-red-50 dark:bg-red-900/20 border border-red-300 dark:border-red-700 font-mono text-red-500 dark:text-red-400">
              h
            </kbd>
            hold
          </span>
      ...
      <div className="flex flex-col gap-2 sm:hidden">
        {sorted.map((item, index) => {
          ...
          return (
            ...
            >
              <div
                className={`flex items-center gap-3 px-4 py-2 border-b border-slate-200 dark:border-white/10
                ${
                  isDone
              ...
              >
                ...
                <span
                  className={`text-xs font-mono font-bold px-2.5 py-0.5 rounded-md ${cfg.badge}`}
                >
                  {item.priority}
                </span>
                <span className={`text-[10px] font-mono ${cfg.code}`}>
                  {item.code}
                </span>
                {isApproved && (
              ...
              >
                <p
                  className={`text-sm font-semibold leading-snug ${isDone ? "text-slate-400 line-through" : "text-slate-900 dark:text-cyan-50"}`}
                >
                  {item.title}
                </p>
            ...
            return (
              ...
              >
                <div className="flex flex-col items-start gap-0.5">
                  <span
                    className={`text-xs font-mono font-bold px-2 py-0.5 rounded-md ${cfg.badge}`}
                  >
                    {item.priority}
                  </span>
                  <span className={`text-[10px] font-mono ${cfg.code}`}>
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  {isApproved && (
                    <span className="flex items-center gap-1 text-xs text-teal-600 dark:text-teal-400 font-bold whitespace-nowrap">
                      <MdCheckCircle size={14} /> {t.approved}
                    </span>
                  )}
                  {isHeld && (
                    <span className="flex items-center gap-1 text-xs text-red-500 dark:text-red-400 font-bold whitespace-nowrap">
                      <MdPause size={14} /> {t.onHold}
                    </span>
                  )}
                  {!isDone && (
                    <>
                      <button
                        onClick={() => doAction(item.id, "approved")}
                        className="text-xs px-2.5 py-1.5 rounded-lg bg-teal-600 text-white font-bold hover:bg-teal-700 active:scale-95 transition-all duration-150 shadow-sm"
                      >
                        {t.approve}
                      </button>
                      <button
                        onClick={() => doAction(item.id, "held")}
                        className="text-xs px-2.5 py-1.5 rounded-lg bg-white dark:bg-transparent text-red-500 dark:text-red-400 font-bold border-2 border-red-400 dark:border-red-500 hover:bg-red-500 hover:text-white active:scale-95 transition-all duration-150"
                      >
                        {t.hold}
                      </button>
                    </>
      ...
      <div className="hidden lg:block">
        <div className="rounded-xl border border-slate-200 dark:border-white/10 overflow-hidden shadow-sm">
          <div className="grid grid-cols-[36px_88px_1fr_auto] gap-3 px-5 py-2 bg-slate-100 dark:bg-slate-800/60 border-b border-slate-200 dark:border-white/10">
            ...
            <span className="text-[10px] font-mono font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest text-center">
              {t.priority}
            </span>
            <span className="text-[10px] font-mono font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest">
              {t.item}
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col items-center gap-1">
                  <span
                    className={`text-xs font-mono font-bold px-2.5 py-1 rounded-md w-full text-center ${cfg.badge}`}
                  >
                    {item.priority}
                  </span>
                  <span className={`text-[10px] font-mono ${cfg.code}`}>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {isApproved && (
                    <span className="flex items-center gap-1 text-xs text-teal-600 dark:text-teal-400 font-bold">
                      <MdCheckCircle size={15} /> {t.approved}
                    </span>
                  )}
                  {isHeld && (
                    <span className="flex items-center gap-1 text-xs text-red-500 dark:text-red-400 font-bold">
                      <MdPause size={15} /> {t.onHold}
                    </span>
                  )}
                  {!isDone && (
                    <>
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          doAction(item.id, "approved");
                        }}
                        className="text-xs px-3 py-1.5 rounded-lg bg-teal-600 text-white font-bold hover:bg-teal-700 active:scale-95 transition-all duration-150 shadow-sm cursor-pointer"
                      >
                        {t.approve}
                      </button>
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          doAction(item.id, "held");
                        }}
                        className="text-xs px-3 py-1.5 rounded-lg bg-white dark:bg-transparent text-red-500 dark:text-red-400 font-bold border-2 border-red-400 dark:border-red-500 hover:bg-red-500 hover:text-white active:scale-95 transition-all duration-150 cursor-pointer"
                      >
                        {t.hold}
                      </button>
                    </>
