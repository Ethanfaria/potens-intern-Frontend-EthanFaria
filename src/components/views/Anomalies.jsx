    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex flex-col gap-3 sm:hidden">
        {translatedAnomalies.map((item) => {
          ...
          return (
            <div key={item.id} className="rounded-xl border border-slate-200 overflow-hidden shadow-sm">
              <div className={`flex items-center gap-3 px-4 py-2.5 border-b border-slate-200 ${cfg.row}`}>
                <span className={`text-xs font-mono font-bold px-2.5 py-0.5 rounded-md shrink-0 ${cfg.badge}`}>
                  {cfg.label}
                </span>
                <span className="text-xs font-bold text-slate-700">{item.system}</span>
                <span className="text-[10px] font-mono text-slate-400 ml-auto">{item.time}</span>
              </div>
      ...
      <div className="hidden sm:block lg:hidden">
        <div className="rounded-xl border border-slate-200 overflow-hidden shadow-sm">
          <div className="grid grid-cols-[80px_70px_1fr_72px] gap-3 px-4 py-2.5 bg-slate-100 border-b border-slate-200">
            <span className="text-[10px] font-mono font-semibold text-slate-500 uppercase tracking-widest">{t.severity}</span>
            <span className="text-[10px] font-mono font-semibold text-slate-500 uppercase tracking-widest">{t.source}</span>
            <span className="text-[10px] font-mono font-semibold text-slate-500 uppercase tracking-widest">{t.detail}</span>
            <span className="text-[10px] font-mono font-semibold text-slate-500 uppercase tracking-widest text-right">{t.impact}</span>
          </div>
          {translatedAnomalies.map((item) => {
            const cfg = severityConfig[item.severity];
            return (
              <div
                key={item.id}
                className={`grid grid-cols-[80px_70px_1fr_72px] gap-3 px-4 py-3.5 items-center border-b border-slate-200 last:border-0 transition-all duration-200 ${cfg.row}`}
              >
                ...
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-slate-900 leading-snug">{item.title}</p>
                  <p className="text-xs text-slate-500 mt-0.5 leading-relaxed line-clamp-1">{item.description}</p>
                </div>
        ...
        <div className="rounded-xl border border-slate-200 overflow-hidden shadow-sm">
          ...
          {translatedAnomalies.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className={`grid grid-cols-[100px_70px_1fr_80px] gap-4 px-5 py-4 items-center border-b border-slate-200 last:border-0 transition-all duration-200 ${cfg.row}`}
              >
