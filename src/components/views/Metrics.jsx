...
// Counts down to 21:00 daily — SLA breach time for PO-78241
...
export default function Metrics() {
  ...
  const urgent = secondsLeft < 3600; // under 1 hour triggers amber state
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div
        className={`grid grid-cols-1 sm:grid-cols-3 ${lowBW ? "gap-2" : "gap-4"}`}
      >
        {/* SLA countdown card */}
        <div
        ...
        >
          ...
          {!lowBW && (
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-3">
              PO-78241 · Vendor payment ₹2.4 Cr · Penalty clause §14b activates
              at 21:00
            </p>
          )}
        </div>
        ...
        <div className={`flex flex-col ${lowBW ? "gap-2" : "gap-4"}`}>
          <div
            className={`rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-800/40 shadow-sm ${lowBW ? "p-3" : "p-4"}`}
          >
            <p className="text-xs font-mono text-slate-500 dark:text-slate-400 uppercase tracking-widest">
              {t.vehiclesOffRadar}
            </p>
            <p
              className={`font-bold text-slate-800 dark:text-cyan-50 mt-1 ${lowBW ? "text-2xl" : "text-3xl"}`}
            >
              3
            </p>
