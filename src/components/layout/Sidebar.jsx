...
const Sidebar = ({ activeView, onNavigate, sidebarOpen, setSidebarOpen }) => {
  ...
  return (
    <>
      ...
      {/* Closes sidebar when user taps outside on mobile */}
      <div
      ...
      {/* Low-BW mode collapses this to an icon-only rail on desktop */}
      <div
        className={`
          ...
          ${
            lowBW
              ? "bg-slate-100 dark:bg-slate-950 w-64 md:w-16"
      ...
      >
        ...
        <div
          ...
          ${lowBW ? "h-12 px-3 md:justify-center" : "h-20 px-6"}`}
        >
          {lowBW ? (
            <div className="flex items-center gap-3 md:justify-center w-full">
              <div className="w-3 h-3 rounded-full bg-teal-400" />
              <h1 className="dark:text-cyan-50 font-bold tracking-wide md:hidden">
                DASHBOARD
              </h1>
            </div>
          ) : (
        ...
        <nav
          className={`flex-1 flex flex-col gap-2 transition-all duration-300
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                title={lowBW ? item.label : undefined} // Tooltip replaces hidden label on desktop rail
              ...
              >
                ...
                {/* Hidden on desktop rail in low-BW; tooltip takes over */}
        ...
        {/* Entire footer is hidden on desktop rail in low-BW mode */}
        <div
          className={`${lowBW ? "md:hidden" : ""} p-4 border-t border-white/10 flex gap-1`}
        >
          <div className="flex-3 flex items-center gap-3 p-3 rounded-xl bg-teal-600/10">
            <div
              className="w-10 h-10 rounded-full bg-gradient-to-br from-teal-400 to-cyan-500
